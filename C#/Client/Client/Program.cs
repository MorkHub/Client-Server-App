...
namespace Client
{
    static class Program
    {
        ...
        static void Broadcast(string command, object data)
        {
        ...
        static void Loop()
        {
            ...
            while (running)
            {
                ...
                SClient client = new SClient(tcpClient.GetStream());
                ...
                Console.WriteLine($@" - {client.Id} connected: {clients.Values}");
                ...
                new Thread(() =>
                {
                    using (Stream stream = tcpClient.GetStream())
                    {
                        try
                        {
                            ...
                            {
                                string[] substrings = msg.Split(new char[] { ' ' }, 2);
                                switch (substrings[0])
                                {
                                    case "PASS":
                                        int passTo;
